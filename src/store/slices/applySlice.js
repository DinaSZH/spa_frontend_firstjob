...
export const {setTestPreviw, setTest,  appendApply, setApplies, removeApply, setApplyStatus } = applySlice.actions

  export const createApplyVacancy = createAsyncThunk('user/createApplyVacancy', async (id, thunkApi) => {
    try {
      const jwt = KeycloakService.getToken(); 
      const {data} = await axios.post(`${POINT_CONTENT}/api/content/vacancies/${id}/apply` ,
      {},
      {
        headers: {
  ...
  export const getTestPreview = createAsyncThunk('user/getTestPreview', async (id, thunkApi) => {
    try {
      const jwt = KeycloakService.getToken(); 
      const  { data } = await axios.get(`${POINT_CONTENT}/api/content/tests/${id}/preview`, {
        headers: {
          Authorization: `Bearer ${jwt}`,
        },
      });
  ...
  export const submitTest = createAsyncThunk('user/submitTest', async ({id, answers}, thunkApi) => {
    try {
      const jwt = KeycloakService.getToken(); 
      const {data} = await axios.post(`${POINT_CONTENT}/api/content/vacancies/${id}/submit` ,
      {answers},
      {
        headers: {
          Authorization: `Bearer ${jwt}`,
